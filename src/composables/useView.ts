...
export default function useView(
  ...
  const adjustCenter = (deltaCoordinates: Coordinate) => view.adjustCenter(deltaCoordinates)
  const adjustResolution = (ratio: number, optAnchor?: Coordinate) => view.adjustResolution(ratio, optAnchor)
  const adjustRotation = (delta: number, optAnchor?: Coordinate) => view.adjustRotation(delta, optAnchor)
  const adjustZoom = (delta: number, optAnchor?: Coordinate) => view.adjustZoom(delta, optAnchor)
  const animate = (...args: (AnimationOptions | ((arg0: boolean) => void))[]) => view.animate(...args)
  const beginInteraction = () => view.beginInteraction()
  const calculateExtent = (optSize?: Size | undefined): Extent => view.calculateExtent(optSize)
  const cancelAnimations = () => view.cancelAnimations()
  const centerOn = (coordinate: Coordinate, size: Size, position: Pixel) => view.centerOn(coordinate, size, position)
  const changed = () => view.changed()
  const dispatchEvent = (event: string | BaseEvent): boolean | undefined => view.dispatchEvent(event)
  ...
  const getResolutionForExtent = (
    extent: Extent,
    size?: Size | undefined,
  ) => view.getResolutionForExtent(extent, size)
  const getResolutionForZoom = (zoom:number):number => view.getResolutionForZoom(zoom)
  ...
  const getZoomForResolution = (resolution:number):number | undefined => view.getZoomForResolution(resolution)

  const setCenter = (center: Coordinate | undefined) => view.setCenter(center)
  const setConstrainResolution = (enabled: boolean) => view.setConstrainResolution(enabled)
  const setMaxZoom = (zoom: number) => view.setMaxZoom(zoom)
  const setMinZoom = (zoom: number) => view.setMinZoom(zoom)
  const setResolution = (resolution: number | undefined) => view.setResolution(resolution)
