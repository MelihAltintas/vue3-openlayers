<template>
    ...
    >
      ...
      <ol-vector-tile-layer ref="vectorTileLayerRef">
        <ol-source-vector-tile :url="url" :format="mvtFormat"> </ol-source-vector-tile>
  ...
  const vectorTileLayerRef = ref<{ vectorTileLayer: VectorTileLayer } | null>(null)
  ...
  watch([highlightingTemplate, bufferRadius], ()=>{
    if(highlightingTemplate.value.length === 0){
      ...
      const bufferedHull = turf.buffer(multiLine, bufferRadius.value, {units: 'meters'})
  
      const format = new GeoJSON()
      const olFeature = format.readFeature(bufferedHull, {dataProjection: 'EPSG:4326', featureProjection: 'EPSG:3857' })
